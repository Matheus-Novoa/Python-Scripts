    ...
    for gas in gases:
        ...
        for root, dirs, files in os.walk(gas):
            ...
            if len(data) != 0:
                ...
                fit_params = map(linregress, data_array[:, index:])
                dc = [p.slope / 6 for p in fit_params]
                dcm[float(key)] = round(np.mean(dc) * 1e-4, 6)

        dcm = dict(sorted(dcm.items()))
        ...
        gas_dict[gas_name] = pd.DataFrame(list(dcm.values()), index=dcm.keys(), columns=['D0 (cm^2/s)'])
